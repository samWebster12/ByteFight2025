...
class SingleProcessByteFightEnv(gym.Env):
    ...
    def update_opponent_model(self, model_params):
        """
        Update the opponent controller using provided model parameters.
        This method instantiates a new PPO model (using the same policy kwargs)
        and sets its parameters from model_params, then wraps it in a local opponent class.
        """
        from stable_baselines3 import PPO
        from custom_extractor import CNNMaskExtractor
        # Use the same policy kwargs as used during training.
        policy_kwargs = {
            "features_extractor_class": CNNMaskExtractor,
            "features_extractor_kwargs": {"features_dim": 512},
            "net_arch": {"pi": [128, 128], "vf": [128, 128]}
        }
        # Create a new PPO model on CPU (so it stays lightweight).
        new_model = PPO("MultiInputPolicy", self, policy_kwargs=policy_kwargs, device="cpu")
        new_model.set_parameters(model_params)
        
        # Define a local opponent wrapper to avoid circular imports.
        class LocalSelfPlayOpponent:
            def __init__(self, model):
                self.model = model
            def play(self, board, time_left):
                # Build observation (same logic as before)
                dim_x = board.game_board.map.dim_x
                dim_y = board.game_board.map.dim_y
                channels = np.zeros((9, dim_y, dim_x), dtype=np.uint8)
                ...
                if mask.sum() == 0:
                    mask[0] = 1
                obs = {"image": image, "action_mask": mask}
                action, _ = self.model.predict(obs, deterministic=True)
                return [action]
        
        # Update the opponent controller.
        self._opponent_controller = LocalSelfPlayOpponent(new_model)
